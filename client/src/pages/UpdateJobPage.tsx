...
export default function UpdateJobPage() {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        ...
        const { data } = await axios.get(`http://localhost:5000/api/v1/job/single/${id}`, {
        ...
        if (data?.job) {
          reset({
            ...
            workType: data.job.workType,
            applyBy: data.job.applyBy?.split("T")[0] || "", // format date
            salary: data.job.salary,
            experience: data.job.experience,
            aboutCompany: data.job.aboutCompany,
            aboutJob: data.job.aboutJob,
            skillsRequired: data.job.skillsRequired,
            additionalInfo: data.job.additionalInfo || "",
          });
        }
      } catch (error) {
        console.error(error);
  ...
  const onSubmit = async (data: JobFormInputs) => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(`http://localhost:5000/api/v1/job/update/${id}`, data, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center py-10 px-4">
      <div className="w-full max-w-4xl bg-white p-8 rounded-2xl shadow-lg border border-blue-200 overflow-y-auto max-h-[90vh]">
        <h2 className="text-3xl font-bold text-blue-700 mb-8 flex items-center gap-3">
          <Briefcase className="w-7 h-7 text-blue-600" /> Update Job
        </h2>
